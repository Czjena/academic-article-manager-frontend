...
export default function ArticlesClient() {
    ...
    const downloadAllPdf = async () => {
        ...
        try {
            const BASE_URL = (process.env.NEXT_PUBLIC_API_URL || "").replace(/\/+$/, "");
            const res = await fetch(`${BASE_URL}/api/articles/export/pdf`, { headers: { Authorization: `Bearer ${token}` } });
            ...
            alert(err instanceof Error ? `Błąd pobierania PDF: ${err.message}` : "Błąd pobierania PDF");
        }
    };
    ...
    if (!isLoggedIn) {
        return (
            <div className="text-center py-20">
                ...
                <Link href="/login" className="rounded-lg bg-blue-600 px-4 py-2 text-white hover:bg-blue-700">Zaloguj się</Link>
            </div>
        );
    }
        ...
        <div className="space-y-4">
            <div className="flex items-center justify-between gap-3">
                ...
                <div className="flex gap-2">
                    <Button onClick={downloadAllPdf} variant="secondary" className="bg-gray-700 hover:bg-gray-600 text-white">
                        Pobierz PDF wszystkich artykułów
                    </Button>
                    <form onSubmit={submitLocal} className="flex gap-2">
                        <Input value={localQ} onChange={e => setLocalQ(e.target.value)} placeholder="Szukaj..." className="w-64" />
                        <Button type="submit">Szukaj</Button>
                        <Link href="/articles/new" className="rounded-lg bg-blue-600 px-3 py-2 hover:bg-blue-700 text-white">
                            Zgłoś artykuł
                        </Link>
                    </form>
                </div>
